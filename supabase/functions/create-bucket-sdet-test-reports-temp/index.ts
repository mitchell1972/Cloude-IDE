...
Deno.serve(async (req) => {
    ...
    const bucketConfig = {
        id: 'sdet-test-reports',
        name: 'sdet-test-reports',
        public: true
    };

    // Add optional configurations
    bucketConfig.allowed_mime_types = ["application/json", "text/html", "text/xml", "application/xml", "text/plain"];
    ...
    const policyQueries = [
        ...
        `CREATE POLICY "Public Access for sdet-test-reports" ON storage.objects FOR SELECT USING (bucket_id = 'sdet-test-reports');`,
        // Allow public to insert objects in this bucket
        `CREATE POLICY "Public Upload for sdet-test-reports" ON storage.objects FOR INSERT WITH CHECK (bucket_id = 'sdet-test-reports');`,
        // Allow public to update objects in this bucket
        `CREATE POLICY "Public Update for sdet-test-reports" ON storage.objects FOR UPDATE USING (bucket_id = 'sdet-test-reports');`,
        // Allow public to delete objects in this bucket
        `CREATE POLICY "Public Delete for sdet-test-reports" ON storage.objects FOR DELETE USING (bucket_id = 'sdet-test-reports');`
    ];
    ...
    return new Response(JSON.stringify({
        ...
        name: 'sdet-test-reports',
        public: true,
        allowed_mime_types: ["application/json", "text/html", "text/xml", "application/xml", "text/plain"],
